...
---
## v0.3.7 (2026-09-23) — 도킹 판정을 좌표 기준으로 변경

v0.3.6 을 현장에 적용해도 반복이 계속됨: 81% 만충 근처에서는 BMS 가 거의 항상 Discharging(0.00A) 을 보고해
"Charging 관측" 자체가 운에 달렸고, 부팅 직후에는 CurrentNodeId 가 없어 이미 도킹돼 있어도 첫 충전 이동을 보냈다.

| 상황 (Idle 만료 시점) | 동작 |
|---|---|
| 지금 Charging, 또는 마지막 충전 이동 이후 Charging 관측 | 트리거 안 함 |
| 현재 pose 가 **충전 노드 등록 좌표 ±0.15m 이내** | 도킹 상태로 판단 — 트리거 안 함 (부팅 직후도 동일). 로그 1회 `충전소 좌표 반경 내 — 도킹 상태로 판단, 재트리거 안 함` |
| 충전 노드 좌표가 매핑에 **미등록** | 판정 불가 — 재트리거 안 함. 로그 1회 `충전 노드 좌표 미등록 … 재트리거 안 함 (설정 화면 매핑에 좌표 입력 권장)` |
| 충전 노드로 기록됐는데 좌표 밖 + Charging 미관측 (9/12 유형 도킹 실패) | 재트리거, 최대 3회 후 경고 1회 |

- 충전 노드 좌표는 설정(`/Settings`) 매핑 표의 X/Y 에 입력 (예: N1001 = 3.61, 14.10). V마커 정밀 도킹 재현성 ~0.05m, 9/12 오판 사례는 0.3m·0.6m.
- 검증(시뮬레이터, N1001 좌표 등록): A) 첫 충전 후 45s 재트리거 없음 — PASS, B) pose 를 좌표 밖으로 옮기면 재트리거 3회 후 정지 — PASS.
